...
def test_reserve_card_from_board():
    ...
    with raises(Exception):
        reserve_card_from_board(game_state, player_n=0, level=4, card_n=0)
    # invalid card
    with raises(Exception):
        reserve_card_from_board(game_state, player_n=0, level=1, card_n=4)
    ...
    with raises(Exception):
        reserve_card_from_board(game_state, player_n=0, level=1, card_n=0)
...
def test_reserve_card_from_deck():
    ...
    # invalid card
    with raises(Exception):
        reserve_card_from_deck(game_state, player_n=0, level=4)

    card = game_state.decks_by_level[0][-1]
    game_state = reserve_card_from_deck(game_state, player_n=0, level=1)
    assert len(game_state.players[0].reserved_cards) == 1
    assert game_state.players[0].reserved_cards[0] == card
    assert game_state.players[0].points == 0
    assert game_state.players[0].gems == Gems(0, 0, 0, 0, 0, 1)
    ...
    # can't reserve more than 3 cards
    with raises(Exception):
        reserve_card_from_deck(game_state, player_n=0, level=1)
